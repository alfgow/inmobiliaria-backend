...
class BotUserAndChatHistoryApiTest extends TestCase
{
    ...
    public function test_find_by_session_endpoint_supports_session_id_or_phone(): void
    {
        $this->withoutMiddleware(AuthenticateApiRequest::class);
        ...
        $this->getJson('/api/v1/bot-users/session/sess-555')
            ->assertOk()
            ->assertJsonPath('data.session_id', 'sess-555');

        $this->getJson('/api/v1/bot-users/session/5215559177781')
            ->assertOk()
            ->assertJsonPath('data.session_id', 'sess-555');

        $this->getJson('/api/v1/bot-users/session/not-found')
            ->assertNotFound()
            ->assertJsonPath('message', 'No se encontró bot_user para la sesión proporcionada.');
    }
