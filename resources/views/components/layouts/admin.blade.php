<!DOCTYPE html>
<html lang="es" x-data="{ darkMode: localStorage.getItem('darkMode') === null ? true : localStorage.getItem('darkMode') === 'true' }" x-init="$watch('darkMode', val => { localStorage.setItem('darkMode', val); document.documentElement.classList.toggle('dark', val); }); document.documentElement.classList.toggle('dark', darkMode);" :class="{ 'dark': darkMode }">
...
<body class="min-h-screen bg-slate-50 text-slate-900 antialiased selection:bg-blue-100 selection:text-blue-900 dark:bg-slate-900 dark:text-slate-100 dark:selection:bg-blue-900 dark:selection:text-blue-100">
    @php
        ...
        $links = [
            ...
            $navLink('settings.api-keys.index', 'API Keys', 'key', 'settings.api-keys.*'),
        ];
        ...
        <aside class="hidden lg:flex flex-col w-72 bg-white border-r border-slate-200 p-6 dark:bg-slate-800 dark:border-slate-700">
            ...
            <nav class="flex-1 space-y-1">
                @foreach ($links as $link)
                    <a href="{{ $link['url'] }}" 
                        ...
                        @elseif($link['icon'] === 'key')
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 7a5 5 0 1 1-9.61 2H3l-1 1v2l2 2h2l1-1v-2h1.39A5 5 0 0 1 15 7Z" />
                                <path stroke-linecap="round" stroke-linejoin="round" d="M18 8h.01M20 10h.01" />
                            </svg>
                        @endif
            ...
            <div class="pt-6 border-t border-slate-100 dark:border-slate-700">
                <a href="{{ route('profile.edit') }}" 
                    @class([
                        'flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 font-medium',
                        'bg-blue-600 text-white shadow-lg shadow-blue-200 dark:shadow-blue-900/30' => request()->routeIs('profile.*', 'password.*', 'appearance.*', 'two-factor.*', 'settings.api-keys.*'),
                        'text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-slate-200' => !request()->routeIs('profile.*', 'password.*', 'appearance.*', 'two-factor.*', 'settings.api-keys.*'),
                    ])>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
